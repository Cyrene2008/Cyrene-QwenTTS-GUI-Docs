...
2. **克隆仓库**: 将 Fork 后的仓库克隆到本地
   ```bash
   git clone https://github.com/您的用户名/Cyrene-QwenTTS-GUI.git
...
1. **更新版本号**: 在 `setup.py` 和相关文件中更新版本号
2. **更新 CHANGELOG**: 更新 CHANGELOG.md 文件，记录本次发布的更改
3. **运行测试**: 确保所有测试都通过
4. **构建发行版**: 构建源代码和二进制发行版
5. **上传发行版**: 将发行版上传到 PyPI
6. **创建 GitHub Release**: 在 GitHub 上创建新的 Release，上传构建的安装包
...
Cyrene QwenTTS GUI 使用分层授权架构。详情请查看 [LICENSE](https://github.com/Cyrene2008/Cyrene-QwenTTS-GUI/blob/main/LICENSE) 文件。
